<#
.SYNOPSIS
    VampKit - Windows 11 Optimization Suite (Shogun Edition v7.3.1)
...
.DESCRIPTION
    ...
    All changes are reversible. Registry snapshots are saved to C:\ProgramData\VampKit\
...
# --- PROJECT VAMPKIT: CONTROLLER v7.3.1 ---
...
Try {
    ...
    if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        ...
        [System.Windows.Forms.MessageBox]::Show(
            "VampKit requires Administrator privileges to modify system settings.`n`n" +
            "Please close this window and run Launch_VampKit.bat instead, or right-click VampKit.ps1 and choose 'Run as Administrator'.",
            "VampKit - Administrator Required",
    ...
    $XamlPath = Join-Path $BaseDir "UI\VampKit.xaml"
    $CorePath = Join-Path $ScriptPath "VampKitCore.ps1"
    $DBPath   = Join-Path $ScriptPath "VampKitDB.ps1"

    $Missing = @()
    if (-not (Test-Path $XamlPath)) { $Missing += "VampKit.xaml" }
    if (-not (Test-Path $CorePath)) { $Missing += "VampKitCore.ps1" }
    if (-not (Test-Path $DBPath))   { $Missing += "VampKitDB.ps1" }

    if ($Missing.Count -gt 0) {
        [System.Windows.Forms.MessageBox]::Show("FATAL ERROR: Missing critical components:`n`n$($Missing -join "`n")`n`nPlease reinstall VampKit.", "Integrity Failure", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        exit
    }
    ...
    $window.Title = "PROJECT VAMPKIT // Definitive Edition (v$Version)"
    $window.Height = 730
    $window.Width = 1100
    ...
    # NOTE: These functions live in VampKitCore.ps1 which is dot-sourced LATER, so guard the calls.
    ...
    $window.Dispatcher.Add_UnhandledException({
        ...
        [System.IO.File]::AppendAllText("$env:ProgramData\VampKit\VampKit_UI_Errors.txt", "[$([DateTime]::Now)] UI ERROR: $Err`r`n")
    ...
    $Global:VampKitDojo = $window.FindName("InfoDojo")
    ...
    if ($SyncHash.Console) {
        ...
        $SyncHash.Console.ToolTip = "Click to open the VampKit logs folder (VampKit.log, diagnostics, state report)"
    ...
    # Now that VampKitCore is loaded, the diagnostic functions exist. Start the session
    ...
    if (Get-Variable "VampKitDB" -ErrorAction SilentlyContinue) { $iss.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry("VampKitDB", $VampKitDB, ""))) }
    ...
    # CRITICAL: inject the global state VampKitCore's functions depend on. Without these the
    ...
    # it). That is why tweak APPLY/REVERT activity never reached VampKit.log -- only the
    # main-thread environment header did. These must be shared into the runspace's globals.
    $VampKitSharedState = @{
        VampKitLogFile   = $Global:VampKitLogFile
        VampKitDiagFile  = $Global:VampKitDiagFile
        VampKitAuditFile = $Global:VampKitAuditFile
        VampKitDiagMode  = $Global:VampKitDiagMode
        SnapshotFile   = $Global:SnapshotFile
        SnapshotCache  = $Global:SnapshotCache
    }
    foreach ($stateKey in $VampKitSharedState.Keys) {
        $iss.Variables.Add((New-Object System.Management.Automation.Runspaces.SessionStateVariableEntry($stateKey, $VampKitSharedState[$stateKey], "")))
    }

    # Inject Core Functions into background memory
    $CoreFunctions = @(
        "Start-VampKitLoop", "Log", "Write-Diag", "Write-DiagException", "Start-DiagSession",
    ...
    $EnginePs.AddScript({
        ...
        Start-VampKitLoop -SyncHash $SyncHash
    })
    ...
    function Bind-InfoDojo {
        ...
        foreach ($c in $ctrls) {
            if (($c -is [System.Windows.Controls.Control]) -and $c.ToolTip -and $c.Tag -ne "Bound") {
                # HOVER
                $c.Add_MouseEnter({ 
                    if ($Global:VampKitDojo -and -not $script:DojoLock) {
                        $t = $this.ToolTip
                        $msg = if ($t -is [System.Windows.Controls.ToolTip]) { $t.Content } else { $t.ToString() }
                        $Global:VampKitDojo.Text = $msg
                        $Global:VampKitDojo.Foreground = [System.Windows.Media.Brushes]::LimeGreen
                    }
                })
                # LEAVE
                $c.Add_MouseLeave({
                    if ($Global:VampKitDojo -and -not $script:DojoLock) {
                        $Global:VampKitDojo.Text = "Hover over any tweak to learn more..."
                        $Global:VampKitDojo.Foreground = [System.Windows.Media.Brushes]::Gray
                    }
                })
                ...
                $c.Add_PreviewMouseRightButtonDown({
                    if ($Global:VampKitDojo) {
                        if ($script:DojoLock) {
                            ...
                            $t = $this.ToolTip
                            $msg = if ($t -is [System.Windows.Controls.ToolTip]) { $t.Content } else { $t.ToString() }
                            $Global:VampKitDojo.Text = "$msg (PINNED - right-click to unpin)"
                            $Global:VampKitDojo.Foreground = [System.Windows.Media.Brushes]::Cyan
    ...
    $window.FindName("Global_TouchMode").Add_Checked({
        try {
            ...
            if ($Global:VampKitDojo) {
                $Global:VampKitDojo.Text = "TOUCH MODE ACTIVE: Interface scaled to $([Math]::Round($factor * 100))% for easier touch input."
                $Global:VampKitDojo.Foreground = [System.Windows.Media.Brushes]::Cyan
            }
        } catch {
            ...
            if ($Global:VampKitDojo) { 
                $Global:VampKitDojo.Text = "Touch Mode error: $($_.Exception.Message)"
                $Global:VampKitDojo.Foreground = [System.Windows.Media.Brushes]::Red
            }
        }
    })
    ...
    $window.FindName("Global_TouchMode").Add_Unchecked({
        try {
            ...
            if ($Global:VampKitDojo) {
                $Global:VampKitDojo.Text = "Standard UI scale restored."
                $Global:VampKitDojo.Foreground = [System.Windows.Media.Brushes]::Gray
            }
        } catch {}
    })
    ...
    $window.FindName("Global_ExpertMode").Add_Checked({ 
        ...
        if ($Global:VampKitDojo) {
            $Global:VampKitDojo.Text = "EXPERT MODE: Dangerous tweaks unlocked. Proceed with caution."
            $Global:VampKitDojo.Foreground = [System.Windows.Media.Brushes]::Red
    ...
    $window.FindName("Global_ExpertMode").Add_Unchecked({ 
        ...
        if ($Global:VampKitDojo) {
            $Global:VampKitDojo.Text = "Standard Mode: Safe optimization profile active."
            $Global:VampKitDojo.Foreground = [System.Windows.Media.Brushes]::Gray
            $script:DojoLock = $false
        }
    })
    ...
    if ($sysHib -and $hhBag) {
        $hibLockAction = {
            if ($sysHib.IsChecked) { 
                ...
                if ($Global:VampKitDojo) {
                    $Global:VampKitDojo.Text = "INTERLOCK ACTIVE: Hot-Bag Fix disabled because Hibernation is OFF."
                    $Global:VampKitDojo.Foreground = [System.Windows.Media.Brushes]::Orange
    ...
    # closes VampKit -- a reboot/shutdown before 100% triggers a BitLocker recovery screen.
    $window.Add_Closing({
        if ($SyncHash.BdeStatus -eq 3) {
            $pct = if ($null -ne $SyncHash.BdePct) { "$($SyncHash.BdePct)%" } else { "in progress" }
            $r = [System.Windows.Forms.MessageBox]::Show(
                "BitLocker is still DECRYPTING your drive ($pct done).`n`nClosing VampKit is OK, but DO NOT restart or shut down the PC until Windows reports decryption is 100% complete (Control Panel -> BitLocker Drive Encryption), or you may hit a BitLocker recovery screen.`n`nClose VampKit anyway?",
    ...
    [System.Windows.Forms.MessageBox]::Show("CRITICAL LAUNCH ERROR:`n`n$errMsg", "VampKit Failed", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
}
