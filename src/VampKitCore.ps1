<#
.SYNOPSIS
    VampKit Core Engine v7.3.1 - Background worker, registry helpers, job dispatcher.

.DESCRIPTION
    Module loaded by VampKit.ps1. Contains:
...
# --- PROJECT VAMPKIT: CORE ENGINE v7.3.1 "SHOGUN EDITION" ---

Add-Type -AssemblyName PresentationFramework, System.Windows.Forms, System.Drawing, WindowsBase

$Global:SnapshotFile = "$env:ProgramData\VampKit\VampKit_Snapshots.json"
$Global:SnapshotCache = @{}

$Global:VampKitLogFile = "$env:ProgramData\VampKit\VampKit.log"
$Global:VampKitDiagFile = "$env:ProgramData\VampKit\VampKit_Diagnostic.log"
$Global:VampKitAuditFile = "$env:ProgramData\VampKit\VampKit_StateReport.txt"
...
$Global:VampKitDiagMode = $true

function Log ($Msg) { 
    # User-facing log: shows in the console panel + persists to VampKit.log
    $Time = Get-Date -Format "HH:mm:ss"
    $FinalMsg = "[$Time] $Msg"
    # Persist to log file so user can review failures after the fact
    try {
        $logDir = Split-Path $Global:VampKitLogFile -Parent
        if (!(Test-Path $logDir)) { New-Item -Path $logDir -ItemType Directory -Force | Out-Null }
        Add-Content -Path $Global:VampKitLogFile -Value $FinalMsg -ErrorAction SilentlyContinue
    } catch {}
...
function Write-Diag ($Msg, $Level = "INFO") {
    ...
    # Writes to VampKit_Diagnostic.log only (never touches the UI, so it's thread-safe and fast).
    if (-not $Global:VampKitDiagMode) { return }
    try {
        ...
        $logDir = Split-Path $Global:VampKitDiagFile -Parent
        if (!(Test-Path $logDir)) { New-Item -Path $logDir -ItemType Directory -Force | Out-Null }
        Add-Content -Path $Global:VampKitDiagFile -Value $line -ErrorAction SilentlyContinue
    } catch {}
}
...
function Start-DiagSession {
    ...
    try {
        ...
        if (!(Test-Path $logDir)) { New-Item -Path $logDir -ItemType Directory -Force | Out-Null }
        $sep = "=" * 78
        Add-Content -Path $Global:VampKitDiagFile -Value "" -ErrorAction SilentlyContinue
        Add-Content -Path $Global:VampKitDiagFile -Value $sep -ErrorAction SilentlyContinue
        Add-Content -Path $Global:VampKitDiagFile -Value "  PROJECT VAMPKIT DIAGNOSTIC SESSION" -ErrorAction SilentlyContinue
        Add-Content -Path $Global:VampKitDiagFile -Value "  Started: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -ErrorAction SilentlyContinue
        Add-Content -Path $Global:VampKitDiagFile -Value $sep -ErrorAction SilentlyContinue
        ...
        Write-Diag "VampKit Version    : 7.3.1" "ENV"
...
function Update-Tweaks {
    ...
    foreach ($k in $VampKitDB.Keys) {
        if ($VampKitDB[$k].Check) {
            try {
                $Status[$k] = & {
    ...
    try {
        ...
        $rep.Add("=" * 72)
        $rep.Add("  PROJECT VAMPKIT v7.3.1 - SYSTEM STATE REPORT")
        ...
        $rep.Add("-" * 72)
        $rep.Add("  Summary: $onCount of $($Status.Count) tweaks active.  Full event log: VampKit.log")
        [System.IO.File]::WriteAllLines($Global:VampKitAuditFile, $rep)
        Write-Diag "State report written ($onCount/$($Status.Count) active) -> $Global:VampKitAuditFile" "ENGINE"
    ...
    $SyncHash.Window.Dispatcher.Invoke([Action]{
        ...
        foreach ($sysKey in $AutoMap.Keys) {
            if (-not $VampKitDB.ContainsKey($sysKey)) { continue }
...
function New-VampKitRestorePoint ($Description, $TimeoutSec = 90) {
    ...
    # move on. Tweaks remain individually reversible via VampKit's own per-tweak snapshots.
    try {
        ...
        if ($h.AsyncWaitHandle.WaitOne([TimeSpan]::FromSeconds($TimeoutSec))) {
            ...
            Log "Restore point is taking too long (Windows System Protection / VSS looks busy or disabled). Skipping it and continuing - every tweak is still individually reversible from within VampKit."
            return $false
        }
    } catch {
        Log "Restore point error: $($_.Exception.Message). Continuing."
        try { Write-DiagException "New-VampKitRestorePoint" $_ } catch {}
        return $false
    }
}

function Start-VampKitLoop ($SyncHash) {
    ...
    if (!(Test-Path "$env:ProgramData\VampKit")) { New-Item -Path "$env:ProgramData\VampKit" -ItemType Directory -Force | Out-Null }
    
    Log "VampKit Core v7.3.1 Shogun Edition Online."

    # --- RE-HOME TWEAK SCRIPTBLOCKS INTO THIS (ENGINE) RUNSPACE ---
    # $VampKitDB's Apply/Revert/Check scriptblocks were created on the UI thread and injected
        ...
        Try {
            ...
            if ($SyncHash.JobQueue.Count -gt 0) {
                ...
                elseif ($job -eq "REVERT_ALL") {
                    ...
                    foreach ($key in $VampKitDB.Keys) { if ($VampKitDB[$key].Revert) { try { Invoke-Command -ScriptBlock $VampKitDB[$key].Revert } catch {} } }
                    Update-Tweaks
                    ...
                    if ($job.Count -gt 0) {
                        ...
                        if ($firstItem -is [PSCustomObject] -or $firstItem -is [System.Collections.DictionaryEntry] -or $firstItem -is [System.Collections.Hashtable]) {
                            ...
                            if ($safeModeRef.Value) {
                                ...
                                New-VampKitRestorePoint "VampKit Pre-Flight" | Out-Null
                            }
                            ...
                            foreach ($taskItem in $job) {
                                ...
                                Try {
                                    $dbEntry = $VampKitDB[$taskItem.Key]
                                    if ($dbEntry) {
                                        if ($dbEntry.Check) {
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
  ...
  <Border Background="#161616" BorderBrush="$accent" BorderThickness="1.5" CornerRadius="6" Width="440">
    <StackPanel Margin="24,20,24,20">
      ...
      <TextBlock Text="PROJECT VAMPKIT  //  BATCH SUMMARY" Foreground="#666" FontFamily="Consolas" FontSize="10" Margin="15,0,0,18"/>
      <Grid>
...
"@
                                    try {
                                        ...
                                        [System.Windows.MessageBox]::Show("Applied: $applied   Reverted: $reverted   Already optimal: $skipped   Failed: $failed", "VampKit - Batch Summary") | Out-Null
                                    }
                                })
                            } catch {}
                            [System.GC]::Collect()
                ...
                elseif ($job -eq "MAINT_RESTORE") { Log "Creating Restore Point (up to 90s)..."; New-VampKitRestorePoint "VampKit Manual Restore" | Out-Null }
                ...
                elseif ($job -eq "MAINT_OPEN_BACKUPS") { Log "Opening Snapshot Folder..."; Invoke-Item "$env:ProgramData\VampKit" -ErrorAction SilentlyContinue }
