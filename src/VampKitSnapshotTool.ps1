<#
.SYNOPSIS
    VampKit - Snapshot Recovery Tool v1.1.0

.DESCRIPTION
    Standalone recovery tool for restoring registry values that VampKit backed up
    before applying tweaks. Reads the snapshot database from
    C:\ProgramData\VampKit\VampKit_Snapshots.json and presents a list of original
...
$SnapshotFile = "$env:ProgramData\VampKit\VampKit_Snapshots.json"

if (!(Test-Path $SnapshotFile)) {
    [System.Windows.Forms.MessageBox]::Show(
        "No VampKit Snapshots found. System is currently in its original state.",
        "VampKit Recovery"
    ) | Out-Null
    exit
}
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="VAMPKIT // Snapshot Recovery" Height="500" Width="800" Background="#0A0A0A" Foreground="White">
    <Grid Margin="20">
        <Grid.RowDefinitions>
